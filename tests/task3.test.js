...
beforeEach(() => {
  const productsFilePath = path.join(__dirname, "..", "src", "data", "task3", "products.json");
  const originalProductsFilePath = path.join(__dirname, "..", "src", "data", "task3", "products.original.json");
  const originalProductsData = fs.readFileSync(originalProductsFilePath);
  fs.writeFileSync(productsFilePath, originalProductsData);
});
